...
public class Main extends javax.swing.JFrame {
    ...
    private void initComponents() {
     ...
     public void saveState(Coordinate start, Coordinate target, Piece chessStart, Piece chessTarget) {
    History move = new History();
    move.setStart(start);
    move.setTarget(target);
    move.setChessStart(chessStart, start);
    move.setChessTarget(chessTarget, target);

    // Nếu không phải là nước đi đầu tiên, xóa các nước đi sau nước đi hiện tại
        ...
        if(checkSelect == false){  
            ...
            if(BoardChess.turn == BoardChess.chessBoard[rowBefor][columBefor].getColor()){
                ...
                if(waitForEnemy == false && Mode == PLAYER_ONLINE)
                    return;
                checkSelect = true;
                // CHECK RESULT: WIN OR LOSE OR HOA  
                checkResult();
...
//                             + "Invalid", "Error", JOptionPane.ERROR_MESSAGE);
                        JOptionPane.showMessageDialog(null, "Checkmate: The game is over.", "Invalid Move", JOptionPane.ERROR_MESSAGE);
...
saveState(pieceSelect, new Coordinate(rowBefor, columBefor), 
          BoardChess.chessBoard[pieceSelect.getRow()][pieceSelect.getCol()], 
          BoardChess.chessBoard[rowBefor][columBefor]);
    ...
    private void btn_UndoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_UndoActionPerformed
         if (currentMove >= 0) { // Kiểm tra xem có nước đi nào để undo không
        History lastMove = history.get(currentMove);

        // Lấy thông tin từ lịch sử
        Coordinate start = lastMove.getStart();
        Coordinate target = lastMove.getTarget();
        Piece chessStart = lastMove.getChessStart();
        Piece chessTarget = lastMove.getChessTarget(); // Sửa lại tên phương thức

        // Khôi phục quân cờ ở vị trí bắt đầu
        BoardChess.chessBoard[start.getRow()][start.getCol()] = chessStart;

        // Khôi phục quân cờ ở vị trí đích
        BoardChess.chessBoard[target.getRow()][target.getCol()] = chessTarget;

        // Giảm vị trí hiện tại trong lịch sử
        currentMove--;
        ...
        JOptionPane.showMessageDialog(null, "No more moves to undo.", "Notification", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    private void btn_RedoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_RedoActionPerformed
        if (currentMove < history.size() - 1) { // Kiểm tra xem có nước đi nào để redo không
        currentMove++;
        History nextMove = history.get(currentMove);

        // Lấy thông tin từ lịch sử
        Coordinate start = nextMove.getStart();
        Coordinate target = nextMove.getTarget();
        Piece chessStart = nextMove.getChessStart();
        Piece chessTarget = nextMove.getChessTarget(); // Sửa lại tên phương thức

        // Di chuyển quân cờ đến vị trí đích
        BoardChess.chessBoard[target.getRow()][target.getCol()] = chessStart;
        BoardChess.chessBoard[start.getRow()][start.getCol()] = new EmptyCell(-1, start);
        ...
        JOptionPane.showMessageDialog(null, "No more moves to redo.", "Notification", JOptionPane.INFORMATION_MESSAGE);
    }
