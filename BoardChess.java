...
//-
    ...
    public static JPanel paintChessBox(Piece pieceSelected,
        ...
        // Kiểm tra xem vua có bị chiếu không (checkmate)
    boolean isCheckmate = BoardChess.checkMate();
    Coordinate kingPosition = null;

    // Tìm quân vua (vua trắng hoặc vua đen)
    for (int iRow = 0; iRow < 8; iRow++) {
        for (int iCol = 0; iCol < 8; iCol++) {
            if (BoardChess.chessBoard[iRow][iCol].getName().equals("White_King") && BoardChess.turn == Piece.White) {
                kingPosition = new Coordinate(iRow, iCol);  // Vị trí của vua trắng
            }
            if (BoardChess.chessBoard[iRow][iCol].getName().equals("Black_King") && BoardChess.turn == Piece.Black) {
                kingPosition = new Coordinate(iRow, iCol);  // Vị trí của vua đen
            }
        }
    }
            ...
            for(int iCol = 1; iCol <= 8; iCol++){
                ...
                if( listPieceGo != null){
                    for(int i=0; i<listPieceGo.size(); i++){
                        ...
                        square.setBorder(BorderFactory.createLineBorder(Color.YELLOW, 2));
                    }
                }
                // Paint color board
                if(iRow %2 != 0){
                ...
                // Nếu có checkmate, làm nổi bật ô của vua bị chiếu
                if (isCheckmate && kingPosition != null && iRow == kingPosition.getRow() + 1 && iCol == kingPosition.getCol() + 1) {
                    square.setBorder(BorderFactory.createLineBorder(Color.RED, 5));  // Viền đỏ dày để nổi bật ô vua bị chiếu
                }
    ...
    public static boolean checkMate(){
        ...
        for(int i = 0; i < listCoordinate.size(); i++){
            ...
            if(listCoordinate.get(i).getRow() == king.getRow() &&
                    listCoordinate.get(i).getCol() == king.getCol()){ square.setBorder(BorderFactory.createLineBorder(Color.RED, 2));
                return true;
            }
        }
        return false;
